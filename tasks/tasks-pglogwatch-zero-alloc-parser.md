...
- [x] T145 [US9] Add `make bench-compare` reproducing every README performance claim, citing corpus version and the machine measured on (GUD-006, VAL-004) — depends on T142
- [~] T146 [US9] ~~Provision the pinned self-hosted benchmark runner with `pgbadger` and `pgweasel` at pinned versions (INF-002, INF-003, SVC-002)~~ — DROPPED. A dedicated runner is infrastructure this project does not need: best-effort benchmarks, measured on a named machine with the clocks fixed for the duration of the run, answer every question it would have. `bench/pinned-run.sh` is that procedure and `bench/MACHINE.md` records the machines. INF-002, INF-003, SVC-002 and TST-008 were amended to match; `bench/RUNNER.md` is deleted.
- [~] T147 [US9] ~~Add `.github/workflows/bench.yml` on the self-hosted runner comparing against the committed baseline with `benchstat`, failing on a > 5 % ns/op regression or any allocation gain (PERF-030, TST-008, AC-020)~~ — REMOVED with T146. A 5 % threshold on shared CI capacity measures variance, not regressions, and a job that cries wolf gets ignored. PERF-030 and AC-020 were amended to a pre-release `benchstat` comparison; the allocation half still runs on every push, because `0 allocs/op` is exact rather than statistical.
